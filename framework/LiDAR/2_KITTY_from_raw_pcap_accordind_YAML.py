...
def print_help_and_exit1():
	print('Usage: .py [from pcap dir] [from preliminary KITTY]')
	sys.exit()
                ...
                for seq_offset in range(0, 1200, 100):
                    ...
                    for step in range(2):
                        ...
                        for i in range(NUM_LASERS): 
                            ...
                            if arr[i * 2] != 0:
                                X,Y,Z,Intens= calc_real_val(arr[i * 2], azimuth, i, arr[i * 2 + 1])
                                #print (X,Y,Z,Intens)
                                if Y>0: 
                                    points.append([X,Y,Z,Intens])
                        seq_index += 1 
...
def main():
    if len(sys.argv) == 3:
        print ('unpack PCAP data:', sys.argv[1], ', to preliminary KITTY :', sys.argv[2])
